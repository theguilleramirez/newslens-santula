...
# Usamos este modelo porque es el más estable y rápido para producción
MODEL_NAME = "gpt-4o-mini"

# Calibración de fuentes para "Historias Útiles"
FEEDS = [
...
ÁNGULO EDITORIAL: (Enfoque humano y estratégico: ¿Por qué esto le importa a alguien hoy?)
...
BAJADA SANTULA: (Cómo aterrizar esto a la realidad cotidiana)
REF. CULTURA: (Una película, libro o frase histórica que ilustre el punto. Si no hay, pon N/A)

Reglas:
- Tono profesional, humano y directo. Sin hype.
- Traduce del inglés al español de forma natural.
"""

# Keywords para detectar impacto cotidiano
IMPACTO_KEYWORDS = [
    "salud", "dinero", "hábitos", "sueño", "cerebro", "hijos", "trabajo", "alimentación",
    "estrés", "longevidad", "bienestar", "apps", "decisiones", "health", "money", "habits"
]
...
def compute_relevance_score(title: str, summary: str) -> float:
    ...
    score = sum(20 for kw in IMPACTO_KEYWORDS if kw in merged)
    return max(0.0, min(100.0, float(score)))

def generate_ai_ficha(title, summary, medio):
    if not CLIENT: return "Error: No hay API Key"
    # LLAMADA DIRECTA SIN FALLBACKS QUE OCULTEN ERRORES
...
def curar_noticias():
    ...
    for feed in FEEDS:
        ...
        try:
            resp = requests.get(feed["url"], headers=HTTP_HEADERS, timeout=20)
            ...
            for entry in parsed.entries[:10]:
                ...
                if score >= 20: # Filtro mínimo de relevancia
                    candidates.append({
    ...
    for item in selected:
        ...
        try:
            ficha = generate_ai_ficha(item["titulo"], item["summary"], item["medio"])
            provider = "openai"
        except Exception as e:
            ficha = f"ERROR DE IA: {str(e)}"
            provider = "error"
